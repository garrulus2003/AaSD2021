...
Point& Polygon::operator[](size_t ind) {
    return vertices[ind];
}

Point Polygon::operator[](size_t ind) const {
    return vertices[ind];
}
...
Polygon Polygon::moved() const {
    ...
    Polygon centered;
    centered.vertices.resize(size());
    for (int i = 0; i < size(); ++i) {
        centered.vertices[i] = vertices[(zero + i) % size()] - vertices[zero];
    }
    return centered;
}
...
Point leftmost(const std::vector<Polygon>& polygons,
    ...
    Point answer;
    for (int i = 0; i < polygons.size(); ++i) {
        answer += polygons[i][first_index[i]];
    }
    return answer;
}
...
std::vector<Point> first_points(const std::vector<Polygon>& polygons,
    ...
    for (int i = 0; i < polygons.size(); ++i) {
        ...
        Point left_end = polygons[i][(first_index[i]) % sz];
        Point right_end = polygons[i][(first_index[i] + 1) % sz];
...
Polygon minkowski(const std::vector<Polygon>& polygons) {
    ...
    answer[0] = leftmost(polygons, first_index);
    ...
    for (int vertex = 1; vertex < vertex_number; ++vertex) {
        for (int i = 0; i < polygons.size(); ++i) {
            ...
            if (current_index[i] < polygons[i].size() && is_best) {
                answer[vertex] = answer[vertex - 1];
                answer[vertex] += current_point[i];
                ++current_index[i];
                size_t sz = polygons[i].size();
                Point left_end =
                        polygons[i][(first_index[i] + current_index[i]) % sz];
                Point right_end =
                        polygons[i][(first_index[i] + current_index[i] + 1) % sz];
...
int main() {
    ...
    Point zero = cities_sum[cities_sum.find_leftmost()];
